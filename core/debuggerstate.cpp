...
bool DebuggerState::AddHardwareBreakpoint(uint64_t address, DebugBreakpointType type, size_t size)
{
	return m_breakpoints->AddHardwareBreakpoint(address, type, size);
}
...
uint32_t DebuggerState::GetRemotePort()
{
	...
	return (uint32_t)settings->Get<uint64_t>("connect.port", data, &scope);
}
...
int32_t DebuggerState::GetPIDAttach()
{
	...
	return (uint32_t)settings->Get<uint64_t>("attach.pid", data, &scope);
}
